...
def current_state(m, flows, by_month):
    to70 = flows.loc[(flows.initial_group == "Group 20") & (flows.final_group == "Group 70"), "share_of_category"].sum()
    to72 = flows.loc[(flows.initial_group == "Group 20") & (flows.final_group == "Group 72"), "share_of_category"].sum()
    stay20 = flows.loc[(flows.initial_group == "Group 20") & (flows.final_group == "Group 20"), "share_of_category"].sum()
    other = 1 - to70 - to72 - stay20
    first = by_month.today_first_time_right.iloc[0], by_month.today_first_time_right.iloc[-1]

    fig, ax = canvas(f"Current state: {m['focus_category']} tickets, {m['core_period_start'][:7]} to {m['core_period_end'][:7]}")
    lanes(ax, [(34, "SERVICE DESK"), (18, "GROUP 20 (first assignment)"), (2, "WHO ACTUALLY RESOLVES IT")])
    box(ax, 4, 38, 22, 8, f"Ticket opened\n{m['focus_incidents'] / m['core_months']:.0f} per month", "white", TEAL)
    box(ax, 32, 38, 26, 8, "Assigned to Group 20\n(default for this category)", "white", TEAL)
    arrow(ax, 26.6, 42, 31.4, 42)
    box(ax, 32, 22, 26, 8, f"Group 20 cannot resolve it\n{m['focus_bounce_rate']:.0%} of tickets change hands", AMBER_SOFT, AMBER, bold=True)
    arrow(ax, 45, 37.6, 45, 30.8, color=AMBER)
    for x, txt, share in [(4, "Group 70", to70), (26, "Group 72", to72), (48, "Group 20 itself", stay20), (70, "Many other\ngroups", other)]:
        box(ax, x, 5, 20, 8, f"{txt}\n{share:.0%} of tickets", "white", AMBER if txt == "Many other\ngroups" else TEAL)
        arrow(ax, 45, 21.6, x + 13, 13.8, color=AMBER)
    ax.text(62, 34.3, f"First time right fell\nfrom {first[0]:.0%} (Mar) to {first[1]:.0%} (May)", fontsize=8.5, color=AMBER,
...
def proposed_state(policy):
    ...
    lanes(ax, [(34, "SERVICE DESK / ASSIGNMENT RULES"), (18, "GROUP 70"), (2, "CONTINUOUS IMPROVEMENT")])
    box(ax, 4, 38, 18, 8, "Ticket opened\n(Category 23)", "white", TEAL)
    box(ax, 30, 38, 30, 8, f"Assignment rule\n{rule_text}", TEAL_SOFT, TEAL, bold=True)
    arrow(ax, 22.6, 42, 29.4, 42)
    box(ax, 30, 22, 30, 8, "Group 70 works the ticket", "white", TEAL)
    arrow(ax, 45, 37.6, 45, 30.8)
    box(ax, 68, 22, 27, 8, f"Resolved by Group 70\n{rule:.0%} of tickets in testing\n(today: {today:.0%})", TEAL_SOFT, TEAL, bold=True)
    ...
    box(ax, 36, 5, 26, 8, "Track first-time-right\nweekly for Category 23", "white", TEAL)
